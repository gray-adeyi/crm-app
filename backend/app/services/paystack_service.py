...
def initialize_transaction(
    *,
    ...
    with httpx.Client(timeout=30) as client:
        res = client.post(
            _url("/transaction/initialize"), headers=_headers(), json=payload
        )
    data = res.json()
    if res.status_code >= 400 or not data.get("status"):
        raise PaystackError(
            data.get("message") or f"Paystack initialize failed ({res.status_code})"
        )
    return data["data"]
...
def verify_transaction(reference: str) -> dict:
    ...
    if res.status_code >= 400 or not data.get("status"):
        raise PaystackError(
            data.get("message") or f"Paystack verify failed ({res.status_code})"
        )
    return data["data"]
...
def disable_subscription(subscription_code: str, email_token: str) -> dict:
    ...
    with httpx.Client(timeout=30) as client:
        res = client.post(
            _url("/subscription/disable"), headers=_headers(), json=payload
        )
    data = res.json()
    if res.status_code >= 400 or not data.get("status"):
        raise PaystackError(
            data.get("message") or f"Paystack disable failed ({res.status_code})"
        )
