...
from app.models.entities import BillingTransaction, Customer, Notification, Order, Product, User
from app.services.plan_entitlements import has_full_dashboard_analytics
from app.schemas.dashboard import CustomerGrowthPoint, DashboardResponse, DeliveryDTO, MonthlyRevenuePoint, RecentOrderDTO
...
def build_dashboard(db: Session, user: User) -> DashboardResponse:
    ...
    total_orders = int(db.query(func.count(Order.id)).filter(Order.user_id == user_id).scalar() or 0)

    total_collected = int(
        db.query(func.coalesce(func.sum(Order.amount_paid), 0)).filter(Order.user_id == user_id).scalar() or 0
    )
    gross_sales = int(db.query(func.coalesce(func.sum(Order.price), 0)).filter(Order.user_id == user_id).scalar() or 0)
    outstanding = int(db.query(func.coalesce(func.sum(Order.balance), 0)).filter(Order.user_id == user_id).scalar() or 0)
    ...
    monthly_revenue = int(
        ...
        .filter(Order.user_id == user_id, Order.created_at.isnot(None), Order.created_at >= m_start, Order.created_at < m_end)
        .scalar()
        or 0
    )

    orders_pending = int(db.query(func.count(Order.id)).filter(Order.user_id == user_id, Order.status == "pending").scalar() or 0)
    orders_partial = int(db.query(func.count(Order.id)).filter(Order.user_id == user_id, Order.status == "partial").scalar() or 0)
    orders_paid = int(db.query(func.count(Order.id)).filter(Order.user_id == user_id, Order.status == "paid").scalar() or 0)
    failed_payments = int(
        db.query(func.count(BillingTransaction.id))
        .filter(BillingTransaction.user_id == user_id, BillingTransaction.status.in_(["failed", "abandoned"]))
        .scalar()
        or 0
    )
    mrr = int(
    ...
    for i in range(5, -1, -1):
        ...
        revenue_by_month.append(MonthlyRevenuePoint(month=label, revenue=rev, orders=ocnt))
    ...
    for o in delivery_rows:
        ...
        if o.delivery_date == today:
            todays.append(dto)
        elif o.delivery_date and o.delivery_date < today and o.status not in {"delivered", "cancelled"}:
            overdue.append(dto)
    ...
    completed_rows = (
        db.query(Order)
        .options(joinedload(Order.customer))
        .filter(Order.user_id == user_id, func.coalesce(Order.fulfillment_type, "delivery") == "delivery", Order.status == "delivered")
        .order_by(Order.id.desc())
        .limit(12)
        .all()
    )
    ...
    out_of_stock_items = int(
        db.query(func.count(Product.id)).filter(Product.user_id == user_id, Product.quantity_in_stock <= 0).scalar() or 0
    )
    low_stock_items = int(
    ...
    if analytics_limited:
        revenue_by_month = revenue_by_month[-3:] if len(revenue_by_month) > 3 else revenue_by_month
        customer_growth = customer_growth[-3:] if len(customer_growth) > 3 else customer_growth
        mrr = 0
        failed_payments = 0
        churn_rate = 0.0
